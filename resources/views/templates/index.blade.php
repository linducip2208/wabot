@extends('layouts.app')
@section('title', 'Template Pesan — WABot')
@section('content')
...
$formatMeta = [
    ...
    'text'     => ['label' => 'Teks Polos', 'icon' => 'fa-align-left', 'cls' => 'bg-gray-100 text-gray-600 border-gray-200', 'iconCls' => 'text-gray-500', 'bgIcon' => 'bg-gray-100'],
...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">Template Pesan</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ $templates->count() }} template · pesan siap pakai untuk kampanye &amp; balasan</p>
    </div>
    <button onclick="openCreate()"
        ...
        <i class="fas fa-plus text-xs"></i> Tambah Template
    </button>
</div>
...
<div class="grid gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift">
        <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                ...
                <form method="POST" action="{{ route('templates.destroy', $t) }}" onsubmit="return confirm('Hapus template ini?')">
                    @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 font-medium mb-1">Belum ada template</p>
        <p class="text-sm text-gray-400 mb-4">Buat template pesan untuk dipakai ulang di kampanye &amp; auto-reply</p>
...
<div id="tplModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="tplModalTitle">Tambah Template</h2>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <select name="format" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                        ...
                        <option value="text">Teks Polos</option>
                        <option value="markdown">Markdown</option>
                    </select>
                </div>
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">Isi Pesan <span class="text-gray-400">(*tebal* _miring_ ~coret~)</span></label>
