...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ $intents->count() }} intent · deteksi maksud pesan pelanggan</p>
    </div>
    <button onclick="openModal()" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2">
        <i class="fas fa-plus text-xs"></i> Intent Baru
    </button>
</div>
...
<div class="grid gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift">
        <div class="flex items-start justify-between gap-3">
            <div class="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium {{ $i->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' }}">{{ $i->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                    </div>
                    <div class="text-xs text-gray-500 mb-1"><span class="text-gray-400">Keywords:</span> {{ $i->keywords }}</div>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                ...
                <form method="POST" action="{{ route('intents.destroy', $i) }}" onsubmit="return confirm('Hapus intent?')">@csrf @method('DELETE')<button class="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600"><i class="fas fa-trash text-xs"></i></button></form>
            </div>
        </div>
    </div>
    @empty
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 font-medium mb-1">Belum ada intent</p>
        <p class="text-sm text-gray-400 mb-4">Deteksi maksud pesan untuk routing otomatis</p>
        <button onclick="openModal()" class="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition"><i class="fas fa-plus text-xs"></i> Intent Baru</button>
    </div>
    @endforelse
</div>
...
<div id="intentModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="intentModalTitle">Intent Baru</h2>
        <form method="POST" action="{{ route('intents.store') }}" class="space-y-3" id="intentForm">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="text-xs font-medium text-gray-500">Intent Label</label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">Keywords <span class="text-gray-400">(pisah koma)</span></label>
                <input type="text" name="keywords" required placeholder="beli, order, pesan, checkout" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">Auto Reply (opsional)</label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('intentModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">Batal</button>
...
function openModal() {
    ...
    document.getElementById('intentModalTitle').textContent = 'Intent Baru';
