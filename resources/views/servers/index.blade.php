@extends('layouts.app')
@section('title', __('common.server') . ' — WABot')
@section('content')

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">{{ __('common.server') }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ __('servers.subtitle') }}</p>
    </div>
    ...
    <button onclick="toggleModal()"
        ...
        <i class="fas fa-plus text-xs"></i> {{ __('common.create') }} {{ __('common.server') }}
    </button>
    @endif
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.total') }} {{ __('common.server') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $servers->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('servers.online') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $servers->filter(fn($s) => $s->is_active)->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.total') }} {{ __('common.session') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $totalSessions }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.message') }}{{ __('servers.per_day') }}</div><div class="text-xl font-extrabold text-gray-900">{{ number_format($todayMessages) }}</div></div>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-5 mb-5">
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fas fa-chart-bar text-brand-500"></i> {{ __('servers.messages_per_server') }}</h2>
        <canvas id="serverMsgChart" height="200"></canvas>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><i class="fas fa-chart-pie text-violet-500"></i> {{ __('servers.sessions_per_server') }}</h2>
        <canvas id="serverSessionChart" height="200"></canvas>
    </div>
</div>
@endif

{{-- {{ __('common.server') }} Cards --}}
<div class="grid gap-4">
        ...
        <div class="p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-2">
                    <span class="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full {{ $online ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600' }}">
                        ...
                        {{ $online ? __('servers.online') : __('servers.offline') }}
                    </span>
                    @if(Auth::user()->isAdmin())
                    ...
                    <form method="POST" action="{{ route('servers.destroy', $s) }}" onsubmit="return confirm('{{ __('common.delete') }} {{ __('common.server') }}?')" class="inline">
                        @csrf @method('DELETE')
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div class="bg-gray-50 rounded-xl p-3 text-center">
                    ...
                    <div class="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{{ __('servers.sessions_online') }}</div>
                </div>
                <div class="bg-gray-50 rounded-xl p-3 text-center">
                    <div class="text-2xl font-extrabold text-gray-700">{{ $totalSess }}</div>
                    <div class="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{{ __('common.total') }} {{ __('common.session') }}</div>
                </div>
                <div class="bg-gray-50 rounded-xl p-3 text-center">
                    <div class="text-2xl font-extrabold text-gray-700">{{ $s->messages_count ?? 0 }}</div>
                    <div class="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{{ __('servers.messages_processed') }}</div>
                </div>
                <div class="bg-gray-50 rounded-xl p-3 text-center">
                    <div class="text-2xl font-extrabold text-gray-700">{{ number_format($s->uptime ?? 0, 1) }}%</div>
                    <div class="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{{ __('servers.uptime_24h') }}</div>
                </div>
            </div>

            @if($totalSess > 0)
            <div class="mt-4 pt-3 border-t border-gray-100">
                <div class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">{{ __('common.session') }} {{ __('common.active') }}</div>
                <div class="flex flex-wrap gap-2">
                    ...
                    @endforeach
                    @if($totalSess > 8) <span class="text-xs text-gray-400 self-center">+{{ $totalSess - 8 }} {{ __('servers.more') }}</span> @endif
                </div>
            </div>
            @endif
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 mb-1 font-medium">{{ __('servers.empty_title') }}</p>
        <p class="text-sm text-gray-400 mb-4">{{ __('servers.empty_desc') }}</p>
...
<div id="serverModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4"><span id="modalTitle">{{ __('common.create') }}</span> {{ __('common.server') }} Baileys</h2>
        <form id="serverForm" method="POST" action="{{ route('servers.store') }}" class="space-y-3">
            @csrf
            <div id="methodField"></div>
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('common.name') }} {{ __('common.server') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('servers.host') }}</label>
                <input type="text" name="host" placeholder="{{ __('servers.host_placeholder') }}" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('servers.port') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('servers.api_key') }}</label>
...
function editServer(id, name, host, port, apiKey) {
    ...
    document.getElementById('modalTitle').textContent = '{{ __('common.edit') }}';
...
new Chart(document.getElementById('serverMsgChart').getContext('2d'), {
    ...
    data: {
        ...
        datasets: [{ label: '{{ __('common.message') }}', data: {!! json_encode($serverMessages) !!}, backgroundColor: ['#3b82f6','#8b5cf6','#06b6d4','#10b981','#f59e0b','#ef4444'], borderRadius: 8 }]
    },
    options: {
        responsive: true,
