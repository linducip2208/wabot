...
<div class="mb-5">
    <h1 class="text-xl font-extrabold text-gray-900">{{ __('logger.page_title') }}</h1>
    <p class="text-sm text-gray-500 mt-0.5">{{ __('logger.subtitle') }}</p>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.total') }} Log</div><div class="text-xl font-extrabold text-gray-900">{{ $logs->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.session') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $logs->where('type','session')->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('sentiment.today') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $logs->where(fn($l) => \Carbon\Carbon::parse($l['created_at'])->isToday())->count() }}</div></div>
    </div>
</div>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                ...
                <th class="px-5 py-3">{{ __('logger.event') }}</th>
                <th class="px-5 py-3 hidden md:table-cell">{{ __('common.detail') }}</th>
                <th class="px-5 py-3 hidden lg:table-cell">{{ __('common.time') }}</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50 transition">
                <td class="px-5 py-3">
                    @if($log['type'] === 'session')
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700"><i class="fas fa-mobile-alt mr-1"></i>{{ __('common.session') }}</span>
                    @else
            ...
            @empty
            <tr><td colspan="4" class="px-5 py-16 text-center text-gray-500">{{ __('logger.no_logs') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
@if($logs->count() > 50)
<div class="text-center mt-3 text-xs text-gray-400">{{ __('logger.showing_recent', ['count' => $logs->count()]) }}</div>
@endif
