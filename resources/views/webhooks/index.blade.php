...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ $webhooks->count() }} endpoint · {{ __('webhooks.subtitle') }}</p>
    </div>
    <button onclick="openCreate()"
        ...
        <i class="fas fa-plus text-xs"></i> {{ __('common.create') }} Webhook
    </button>
</div>
...
<div class="grid gap-3 mb-8">
        ...
        <div class="flex items-start justify-between gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium {{ $w->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' }}">
                            ...
                            {{ $w->is_active ? __('common.active') : __('common.inactive') }}
                        </span>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                <form method="POST" action="{{ route('webhooks.test', $w) }}">
                    @csrf
                    <button class="p-1.5 rounded-lg hover:bg-sky-50 text-gray-400 hover:text-sky-600" title="{{ __('webhooks.send_test') }}"><i class="fas fa-vial text-xs"></i></button>
                </form>
                ...
                <form method="POST" action="{{ route('webhooks.destroy', $w) }}" onsubmit="return confirm('{{ __('webhooks.delete_confirm') }}')">
                    @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 font-medium mb-1">{{ __('webhooks.empty_title') }}</p>
        <p class="text-sm text-gray-400 mb-4">{{ __('webhooks.empty_desc') }}</p>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-4 py-3 border-b border-gray-200 flex items-center gap-2">
        ...
        <h2 class="text-sm font-bold text-gray-900">{{ __('webhooks.recent_logs') }}</h2>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left text-[11px] font-semibold uppercase tracking-wider text-gray-400 border-b border-gray-100">
                    <th class="px-4 py-2.5">{{ __('common.time') }}</th>
                    <th class="px-4 py-2.5">{{ __('webhooks.webhook') }}</th>
                    <th class="px-4 py-2.5">{{ __('webhooks.event') }}</th>
                    <th class="px-4 py-2.5">{{ __('common.status') }}</th>
                    <th class="px-4 py-2.5">{{ __('webhooks.description_label') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/60">
                    ...
                    <td class="px-4 py-2.5">
                        @if($code === null)
                            <span class="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-md bg-rose-50 text-rose-700 border border-rose-200"><i class="fas fa-xmark text-[9px]"></i> {{ __('common.failed') }}</span>
                        @else
                ...
                <tr>
                    <td colspan="5" class="px-4 py-10 text-center text-gray-400 text-sm">
                        <i class="fas fa-inbox text-2xl mb-2 block opacity-40"></i> {{ __('webhooks.empty_logs') }}
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div id="whModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="whModalTitle">{{ __('common.create') }} Webhook</h2>
        <form method="POST" action="{{ route('webhooks.store') }}" class="space-y-3" id="whForm">
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('webhooks.url_endpoint') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500 mb-1.5 block">{{ __('webhooks.events_to_send') }}</label>
...
function openCreate() {
    ...
    document.getElementById('whModalTitle').textContent = '{{ __('common.create') }} Webhook';
...
function editWebhook(id) {
    ...
    if (!data) return;
    const f = document.getElementById('whForm');
    document.getElementById('whModalTitle').textContent = '{{ __('webhooks.edit_webhook') }}';
