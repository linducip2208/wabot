...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ __('payouts.subtitle') }}</p>
    </div>
    <button onclick="toggleModal()"
        ...
        <i class="fas fa-plus text-xs"></i> {{ __('payouts.request') }}
    </button>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.pending') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $payouts->where('status','pending')->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.completed') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $payouts->where('status','completed')->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center"><i class="fas fa-times-circle text-red-500"></i></div>
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">{{ __('common.rejected') }}</div><div class="text-xl font-extrabold text-gray-900">{{ $payouts->where('status','rejected')->count() }}</div></div>
    </div>
</div>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                ...
                <th class="px-5 py-3 hidden md:table-cell">{{ __('payouts.account_info') }}</th>
                <th class="px-5 py-3">{{ __('common.status') }}</th>
                <th class="px-5 py-3 hidden lg:table-cell">{{ __('common.submitted') }}</th>
                <th class="px-5 py-3 hidden lg:table-cell">{{ __('payouts.processed') }}</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            ...
            @empty
            <tr><td colspan="7" class="px-5 py-16 text-center text-gray-500">{{ __('payouts.no_payouts') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
<div id="payoutModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4">{{ __('payouts.request_title') }}</h2>
        <form method="POST" action="{{ route('payouts.store') }}" class="space-y-3">
            ...
            <div>
                ...
                <select name="method" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                    ...
                    <option value="bank_transfer">{{ __('payouts.bank_transfer') }}</option>
                </select>
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('payouts.account_info') }}</label>
                <textarea name="account_info" rows="2" required placeholder="{{ __('payouts.account_info_placeholder') }}"
            ...
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700">{{ __('payouts.submit') }}</button>
            </div>
        </form>
    </div>
</div>
