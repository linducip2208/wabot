@extends('layouts.app')
@section('title', 'Button Baru — WABot')
@section('content')
...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-xl" x-data="btnForm({{ old('buttons') ? old('buttons') : '[]' }})">
    <form method="POST" action="{{ route('buttons.store') }}" class="space-y-4" @submit="syncButtons">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="header_text" value="{{ old('header_text') }}" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Header Media URL (opsional)</label>
            <input type="url" name="header_media_url" value="{{ old('header_media_url') }}" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Body Text</label>
            <textarea name="body_text" rows="3" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">{{ old('body_text') }}</textarea>
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Footer Text (opsional)</label>
            <input type="text" name="footer_text" value="{{ old('footer_text') }}" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
