@extends('layouts.app')
@section('title', 'Pembayaran — WABot')
@section('content')

<div class="max-w-2xl mx-auto">
    <a href="{{ route('plans.index') }}" class="text-sm text-gray-500 hover:text-brand-600">&larr; Kembali</a>
    <h1 class="text-2xl font-extrabold text-gray-900 mt-1 mb-2">Pembayaran</h1>
    <p class="text-gray-500 mb-6">Paket {{ $plan->name }} — Rp {{ number_format($plan->price, 0, ',', '.') }}</p>

    <div x-data="paymentForm()" class="space-y-5">
        {{-- Gateway Selection --}}
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <h2 class="font-bold text-gray-900 mb-3">Pilih Metode Pembayaran</h2>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5" x-show="gatewayId" x-transition>
            <h2 class="font-bold text-gray-900 mb-3">Instruksi Pembayaran</h2>
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="text-sm text-gray-700 whitespace-pre-line leading-relaxed" x-text="selectedInstructions || 'Pilih metode pembayaran terlebih dahulu.'"></div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('payment.upload', $subscription) }}" class="bg-white rounded-xl border border-gray-200 p-5" x-show="gatewayId" x-transition>
            ...
            <h2 class="font-bold text-gray-900 mb-3">Konfirmasi Pembayaran</h2>
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800 mb-4">
                <i class="fas fa-info-circle mr-1"></i> Setelah transfer, klik "Konfirmasi Pembayaran". Admin akan verifikasi.
            </div>
