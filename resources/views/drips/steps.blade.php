@extends('layouts.app')
@section('title', __('drips.steps_title', ['name' => $dripCampaign->name]))
@section('content')
...
<div class="flex items-center gap-3 mb-5">
    ...
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">{{ __('drips.steps_title', ['name' => $dripCampaign->name]) }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ __('drips.steps_subtitle', ['count' => $dripCampaign->steps->count()]) }}</p>
    </div>
</div>
...
<div class="grid lg:grid-cols-3 gap-4">
    <div class="lg:col-span-2 space-y-3">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        <div class="text-xs text-gray-400 mb-1"><i class="fas fa-hourglass-half mr-1"></i> {{ __('drips.wait_hours', ['hours' => $step->wait_hours]) }}
                        ...
                        @if($step->media_url)<a href="{{ $step->media_url }}" target="_blank" class="text-xs text-brand-600 hover:underline"><i class="fas fa-paperclip mr-1"></i> {{ __('common.view') }} media</a>@endif
                    </div>
                </div>
                <form method="POST" action="{{ route('drips.steps.destroy', [$dripCampaign, $step]) }}" onsubmit="return confirm('{{ __('common.delete') }} step?')">
                    @csrf @method('DELETE')
        ...
        <div class="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
            ...
            <p class="text-gray-500">{{ __('drips.empty_steps') }}</p>
        </div>
        @endforelse
    </div>

    <div class="bg-white rounded-xl border border-gray-200 p-5 h-fit">
        <h2 class="font-bold text-gray-900 mb-3">{{ __('drips.create_step') }}</h2>
        <form method="POST" action="{{ route('drips.steps.store', $dripCampaign) }}" class="space-y-3">
            @csrf
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <label class="text-xs font-medium text-gray-500">{{ __('drips.step_order') }}</label>
                ...
                <div>
                    <label class="text-xs font-medium text-gray-500">{{ __('drips.wait_hours_label') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('common.message') }}</label>
                <textarea name="message" rows="3" required class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"></textarea>
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('drips.media_url_optional') }}</label>
                <input type="url" name="media_url" placeholder="https://..." class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">{{ __('drips.ai_key_optional') }}</label>
                <select name="ai_key_id" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
                    <option value="">{{ __('drips.no_ai') }}</option>
                    @foreach($aiKeys as $k)<option value="{{ $k->id }}">{{ $k->name }}</option>@endforeach
                </select>
            </div>
            <button type="submit" class="w-full bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700"><i class="fas fa-plus mr-1"></i> {{ __('drips.create_step') }}</button>
        </form>
    </div>
</div>
@endsection
