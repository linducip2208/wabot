@extends('layouts.app')
@section('title', __('drips.index_title'))
@section('content')

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">{{ __('drips.heading') }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ __('drips.subtitle', ['count' => $campaigns->count()]) }}</p>
    </div>
    <a href="{{ route('drips.create') }}" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2">
        <i class="fas fa-plus text-xs"></i> {{ __('drips.new_drip') }}
    </a>
</div>
...
<div class="grid gap-3">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500">{{ $c->session?->name ?? '-' }} · {{ $c->steps_count }} {{ __('drips.steps_label') }}</div>
                </div>
            </div>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('drips.steps', $c) }}" class="text-[11px] bg-teal-50 text-teal-700 hover:bg-teal-100 px-2.5 py-1.5 rounded-lg font-medium"><i class="fas fa-list-ol"></i> {{ __('drips.steps_label') }}</a>
                ...
                <form method="POST" action="{{ route('drips.destroy', $c) }}" class="inline" onsubmit="return confirm('{{ __('common.delete') }} {{ __('drips.heading') }}?')">
                    @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 font-medium mb-1">{{ __('drips.empty_title') }}</p>
        <p class="text-sm text-gray-400 mb-4">{{ __('drips.empty_subtitle') }}</p>
        <a href="{{ route('drips.create') }}" class="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition"><i class="fas fa-plus text-xs"></i> {{ __('drips.empty_cta') }}</a>
    </div>
    @endforelse
</div>
@endsection
