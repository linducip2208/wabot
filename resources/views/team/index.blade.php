...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ $members->count() }} anggota tim · agen customer service</p>
    </div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50">
                ...
                <td class="px-5 py-3"><span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium {{ $m->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' }}"><span class="w-1.5 h-1.5 rounded-full {{ $m->is_active ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>{{ $m->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                <td class="px-5 py-3 text-right">
                    ...
                    <form method="POST" action="{{ route('team-members.destroy', $m) }}" class="inline" onsubmit="return confirm('Hapus anggota?')">@csrf @method('DELETE')<button class="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600"><i class="fas fa-trash text-xs"></i></button></form>
                </td>
            </tr>
            @empty
            <tr><td colspan="6" class="px-5 py-16 text-center">
                ...
                <p class="text-sm text-gray-400 mt-1">Tambahkan agen untuk menangani percakapan</p>
            </td></tr>
            @endforelse
        </tbody>
    </table>
...
<div id="memberModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="memberModalTitle">Tambah Anggota</h2>
        <form method="POST" action="{{ route('team-members.store') }}" class="space-y-3" id="memberForm">
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">Password <span class="text-gray-400" id="pwdHint">(min 6 karakter)</span></label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('memberModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">Batal</button>
