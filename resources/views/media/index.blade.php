@extends('layouts.app')
@section('title', 'Media Templates — WABot')
@section('content')

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">Media Templates</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ $templates->count() }} template · media siap kirim (gambar, video, dokumen, lokasi)</p>
    </div>
    <button onclick="openModal()" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2">
        <i class="fas fa-plus text-xs"></i> Media Baru
    </button>
</div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift">
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <form method="POST" action="{{ route('media-templates.destroy', $t) }}" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600"><i class="fas fa-trash text-xs"></i></button></form>
                </div>
            </div>
...
<div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
    ...
    <p class="text-gray-500 font-medium mb-1">Belum ada media template</p>
    <p class="text-sm text-gray-400 mb-4">Simpan media untuk dikirim berulang kali</p>
    <button onclick="openModal()" class="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition"><i class="fas fa-plus text-xs"></i> Media Baru</button>
</div>
@endforelse
...
<div id="mediaModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()" x-data="{ type: 'image' }">
        <h2 class="text-lg font-bold mb-4" id="mediaModalTitle">Media Baru</h2>
        <form method="POST" action="{{ route('media-templates.store') }}" class="space-y-3" id="mediaForm">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <select name="type" x-model="type" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                        <option value="image">Gambar</option><option value="video">Video</option><option value="audio">Audio</option><option value="document">Dokumen</option><option value="sticker">Sticker</option><option value="location">Lokasi</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">Caption (opsional)</label>
                <textarea name="caption" rows="2" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm"></textarea>
            </div>
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('mediaModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">Batal</button>
...
function openModal() {
    ...
    document.getElementById('mediaModalTitle').textContent = 'Media Baru';
