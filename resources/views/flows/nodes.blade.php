...
<div class="flex items-center justify-between mb-5">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">{{ __('flows.subtitle_nodes') }} <span class="font-mono bg-gray-100 px-1.5 rounded">{{ $flow->trigger_keyword }}</span></p>
        </div>
    </div>
</div>
...
<div x-data="flowBuilder()" class="space-y-4">
    <div class="flex flex-wrap items-center gap-2 bg-white rounded-xl border border-gray-200 p-3">
        <span class="text-xs font-semibold text-gray-500 mr-1">{{ __('common.create') }} {{ __('flows.nodes') }}:</span>
        <button type="button" @click="addNode('message')" class="text-xs bg-sky-50 text-sky-700 hover:bg-sky-100 px-3 py-1.5 rounded-lg font-medium"><i class="fas fa-comment-dots mr-1"></i> {{ __('flows.node_type_message') }}</button>
    ...
    <form method="POST" action="{{ route('flows.nodes.store', $flow) }}" @submit="prepareSubmit">
            ...
            <template x-for="(node, idx) in nodes" :key="idx">
                <div class="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <div class="grid gap-3">
                        <div>
                            <label class="text-xs font-medium text-gray-500">{{ __('flows.node_label') }}</label>
                            <input type="text" x-model="node.label" required placeholder="{{ __('flows.node_label_placeholder') }}" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
                        </div>
                        {{-- message / ai --}}
                        ...
                        <template x-if="node.type==='condition'">
                            <div class="grid grid-cols-3 gap-2">
                                <input type="text" x-model="node.condition_field" placeholder="{{ __('flows.node_field_placeholder') }}" class="rounded-xl border border-gray-300 px-3 py-2 text-sm">
                                ...
                                <input type="text" x-model="node.condition_value" placeholder="{{ __('flows.node_value') }}" class="rounded-xl border border-gray-300 px-3 py-2 text-sm">
                            </div>
                        </template>
                        <template x-if="node.type==='wait'">
                            <div>
                                <label class="text-xs font-medium text-gray-500">{{ __('flows.node_wait_seconds') }}</label>
        ...
        <div x-show="nodes.length===0" class="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
            ...
            <p class="text-gray-500">{{ __('flows.empty_nodes') }}</p>
        </div>

        <div class="mt-4 flex justify-end">
            <button type="submit" x-show="nodes.length>0" class="bg-brand-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700"><i class="fas fa-save mr-1"></i> {{ __('flows.save_flow') }}</button>
        </div>
    </form>
</div>
