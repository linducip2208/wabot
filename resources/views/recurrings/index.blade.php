...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50 transition">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center gap-1">
                        <button onclick="editSchedule({{ $s->id }}, '{{ $s->name }}', '{{ $s->recurrence }}', '{{ $s->time }}', {{ $s->day_of_week ?? 0 }}, {{ $s->day_of_month ?? 1 }}, '{{ addslashes($s->message) }}', '{{ $s->target_type }}')" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-brand-600"><i class="fas fa-edit text-xs"></i></button>
            ...
            @empty
            <tr><td colspan="8" class="px-4 py-16 text-center text-gray-500">Belum ada jadwal berulang. <button onclick="document.getElementById('createModal').classList.remove('hidden')" class="text-brand-600 hover:underline">Buat sekarang</button></td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
<div id="createModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4">Tambah Jadwal Berulang</h2>
        <form method="POST" action="{{ route('recurrings.store') }}" class="space-y-3">
            @csrf
            <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2">
                    <label class="text-xs font-medium text-gray-500">Nama</label>
                    <input type="text" name="name" placeholder="Welcome Message" required class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                </div>
                <div>
                    <label class="text-xs font-medium text-gray-500">Pengulangan</label>
                    <select name="recurrence" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <input type="time" name="time" value="08:00" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="text-xs font-medium text-gray-500">Sesi</label>
                    <select name="session_id" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <select name="target_type" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm">
            ...
            <div>
                ...
                <textarea name="message" rows="3" required placeholder="Halo {name}! {Selamat datang|Hai, apa kabar?}" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500"></textarea>
            </div>
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('createModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">Batal</button>
