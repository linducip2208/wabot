@extends('layouts.app')
@section('title', 'SLA Dashboard — WABot')
@section('content')
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 card-lift">
        ...
        <div class="text-2xl font-extrabold text-emerald-600" id="statCompliance">{{ $compliance }}%</div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-5 card-lift">
        ...
        <div class="text-2xl font-extrabold text-red-600" id="statFrBreach">{{ $frBreach }}</div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-5 card-lift">
        ...
        <div class="text-2xl font-extrabold text-gray-900" id="statAvg">{{ $stats['avg_first_response_minutes'] ?? 0 }} <span class="text-sm text-gray-400 font-medium">mnt</span></div>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-4">
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="space-y-3">
            <div>
                <div class="flex items-center justify-between text-sm mb-1"><span class="text-gray-600">Respons pertama</span><span class="font-semibold text-red-600">{{ $frBreach }} / {{ $total }}</span></div>
                <div class="h-2 rounded-full bg-gray-100 overflow-hidden"><div class="h-full bg-red-500" style="width: {{ $total > 0 ? min(100, $frBreach / $total * 100) : 0 }}%"></div></div>
            </div>
            <div>
                <div class="flex items-center justify-between text-sm mb-1"><span class="text-gray-600">Penyelesaian</span><span class="font-semibold text-orange-600">{{ $resBreach }} / {{ $total }}</span></div>
                <div class="h-2 rounded-full bg-gray-100 overflow-hidden"><div class="h-full bg-orange-500" style="width: {{ $total > 0 ? min(100, $resBreach / $total * 100) : 0 }}%"></div></div>
            </div>
        </div>
    </div>
    ...
    if (ctx && window.Chart) {
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [{{ max(0, $total - $frBreach - $resBreach) }}, {{ $frBreach }}, {{ $resBreach }}],
