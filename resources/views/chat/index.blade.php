    ...
    <aside class="w-full md:w-80 lg:w-96 flex-shrink-0 bg-gray-900 flex flex-col border-r border-gray-800"
        ...
        <nav class="flex-1 overflow-y-auto" id="contactList">
                ...
                <div @click="openChat(c)"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span @click.stop="openChat(c); $nextTick(() => showEditModal = true)" x-show="!c.display_phone && !c.is_lid" class="text-[11px] text-orange-400 cursor-pointer hover:underline">+ Set nomor HP</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-xs text-gray-400 truncate" x-text="c.last_message || 'Belum ada pesan'"></span>
                        </div>
                    </div>
                </div>
            </template>
            <div x-show="filteredContacts.length === 0" class="px-4 py-16 text-center">
                ...
                <p class="text-gray-600 text-xs mt-1">Pesan akan muncul di sini setelah ada yang chat</p>
            </div>
        </nav>
    ...
    <main class="flex-1 flex flex-col bg-[#efeae2]"
        ...
        <div x-show="!activeContact" class="flex-1 flex items-center justify-center">
            <div class="text-center">
                ...
                <p class="text-gray-500 text-sm max-w-xs">Pilih kontak dari sidebar untuk mulai chat.<br>Auto-reply aktif untuk keyword yang cocok.</p>
            </div>
        </div>
            ...
            <div class="flex-1 flex flex-col h-full">
                ...
                <div class="flex items-center gap-3 bg-[#f0f2f5] px-4 py-2.5 border-b border-gray-200">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            ...
                            <button @click.stop="showEditModal = true" class="p-0.5 rounded hover:bg-gray-200 transition flex-shrink-0" title="Edit kontak">
                        ...
                        <span x-show="!activeContact.display_phone && !activeContact.is_lid" @click="showEditModal = true" class="text-xs text-orange-500 cursor-pointer hover:underline">+ Set nomor HP</span>
                        <div class="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
                            <span x-show="sessionId">Online via <span x-text="sessionName"></span></span>
                            <span x-show="!sessionId" class="text-red-500">Tidak ada sesi terhubung</span>
                            <template x-if="autoreplies.length > 0">
                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-purple-100 text-purple-700">
                                    ...
                                    <span x-text="autoreplies.length"></span> auto-reply aktif
                                </span>
                            </template>
                        </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto px-4 md:px-12 py-4 space-y-1.5" id="messageArea"
                            ...
                            <div class="max-w-[75%] md:max-w-[60%]">
                                ...
                                <div class="flex items-center gap-1.5 mt-0.5"
                                    ...
                                    <template x-if="msg.direction === 'out'">
                                        <svg class="w-3 h-3" :class="msg.status === 'sent' ? 'text-gray-400' : 'text-blue-500'" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div x-show="messages.length === 0" class="flex items-center justify-center h-full">
                        <p class="text-gray-500 text-sm">Belum ada pesan. Kirim pesan pertama!</p>
                    </div>
                </div>
...
<div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
    ...
    <div class="relative bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" @click.stop="">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Edit Kontak</h3>
        <div class="space-y-3">
            <div>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">Nomor HP (untuk tampilan)</label>
                ...
                <p class="text-[10px] text-gray-400 mt-1">Hanya untuk tampilan, tidak ubah JID pengiriman</p>
            </div>
        </div>
        <div class="flex gap-2 mt-4">
            <button @click="showEditModal = false" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2 text-sm font-medium hover:bg-gray-200">Batal</button>
            <button @click="saveContact()" class="flex-1 bg-brand-600 text-white rounded-xl py-2 text-sm font-semibold hover:bg-brand-700">Simpan</button>
        </div>
    </div>
</div>
@endsection
