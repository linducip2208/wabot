@extends('layouts.app')
@section('title', __('telegram.title') . ' — WABot')

@section('content')
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900">{{ __('telegram.title') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('telegram.subtitle') }}</p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            ...
            <i class="fas fa-plus text-xs"></i> {{ __('telegram.create_bot') }}
        </button>
    </div>
    ...
    @if($accounts->isEmpty())
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1">{{ __('telegram.empty_title') }}</h3>
            <p class="text-sm text-gray-400 mb-4">{{ __('telegram.empty_desc') }}</p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                class="bg-sky-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-sky-700 transition">
                {{ __('telegram.create_bot') }}
            </button>
        </div>
    @else
        <div class="space-y-3">
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4">
                        ...
                        <div class="flex items-center gap-1.5">
                            @if($acc->status !== 'connected')
                                <form action="{{ route('telegram.connect', $acc) }}" method="POST">
                                    @csrf
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-sky-600 text-white hover:bg-sky-700 transition">
                                        <i class="fab fa-telegram mr-1"></i>{{ __('common.connect') }}
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('telegram.disconnect', $acc) }}" method="POST">
                                    @csrf
                            ...
                            <form action="{{ route('telegram.destroy', $acc) }}" method="POST" onsubmit="return confirm('{{ __('common.delete') }}?')" class="inline">
                                @csrf @method('DELETE')
    ...
    <div class="mt-4 bg-sky-50 rounded-xl border border-sky-100 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1">{{ __('telegram.setup_title') }}</h3>
                <p class="text-xs text-gray-500">{!! __('telegram.setup_desc') !!}</p>
            </div>
        </div>
    </div>
</div>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('telegram.create_bot') }}</h2>
        <form action="{{ route('telegram.store') }}" method="POST" class="space-y-3">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('telegram.bot_token') }}</label>
            ...
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-sky-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-sky-700">{{ __('common.save') }}</button>
            </div>
        </form>
    </div>
</div>
...
<div id="testModal{{ $acc->id }}" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        ...
        <form action="{{ route('telegram.test', $acc) }}" method="POST" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('telegram.chat_id') }}</label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('testModal{{ $acc->id }}').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">{{ __('common.cancel') }}</button>
                <button type="submit" class="flex-1 bg-sky-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-sky-700">{{ __('common.send') }}</button>
            </div>
        </form>
    </div>
</div>
