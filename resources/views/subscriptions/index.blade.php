...
<div class="mb-5">
    <h1 class="text-xl font-extrabold text-gray-900">Langganan Saya</h1>
    <p class="text-sm text-gray-500 mt-0.5">Kelola paket dan lihat riwayat langganan Anda</p>
</div>
            ...
            <div>
                ...
                <div class="text-xs text-brand-600 mt-0.5">
                    @if($currentSubscription->ends_at)
                        <span><i class="fas fa-calendar-alt mr-1"></i> Berlaku hingga {{ $currentSubscription->ends_at->format('d M Y') }}</span>
                    @else
                        <span><i class="fas fa-calendar-alt mr-1"></i> Aktif selamanya</span>
                    @endif
                </div>
                @endif
            </div>
...
<div class="mb-6 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 flex items-center gap-3">
    ...
    <div>
        <span class="text-sm text-amber-800 font-medium">Anda belum memiliki paket aktif.</span>
        <a href="{{ route('plans.index') }}" class="text-sm text-brand-600 font-semibold hover:underline ml-1">Pilih paket sekarang</a>
    </div>
</div>
@endif
...
<div class="grid md:grid-cols-3 gap-3 mb-6">
    @foreach([
        ['fas fa-mobile-alt', 'Sesi WhatsApp', $usage['sessions']['current'], $usage['sessions']['limit'], 'bg-sky-50 text-sky-500'],
        ['fas fa-address-book', 'Kontak', $usage['contacts']['current'], $usage['contacts']['limit'], 'bg-violet-50 text-violet-500'],
...
<div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6 card-lift">
    <h2 class="font-bold text-gray-900 mb-1 flex items-center gap-2"><i class="fas fa-ticket-alt text-brand-500"></i> Tukar Voucher</h2>
    <p class="text-sm text-gray-500 mb-4">Punya kode voucher? Tukarkan di sini untuk mendapatkan akses paket.</p>
    <form method="POST" action="{{ route('vouchers.redeem') }}" class="flex items-center gap-3 max-w-lg">
        @csrf
        <input type="text" name="code" placeholder="Masukkan kode 12 karakter" required
        ...
        <button type="submit" class="bg-brand-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2 flex-shrink-0">
            <i class="fas fa-check text-xs"></i> Tukar
        </button>
    </form>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <h2 class="font-bold text-gray-900 flex items-center gap-2"><i class="fas fa-history text-brand-500"></i> Riwayat Langganan</h2>
        <span class="text-xs text-gray-400">{{ $subscriptions->count() }} catatan</span>
    </div>
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                <th class="px-5 py-3">Paket</th>
                <th class="px-5 py-3 hidden md:table-cell">Mulai</th>
                <th class="px-5 py-3 hidden md:table-cell">Berakhir</th>
            ...
            <tr class="hover:bg-gray-50/50 transition">
                <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium text-gray-900 text-xs">{{ $sub->plan?->name ?? 'Tanpa Paket' }}</span>
                    </div>
                </td>
                ...
                <td class="px-5 py-3">
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full
                        ...
                        {{ $sub->status === 'active' ? 'Aktif' : ($sub->status === 'expired' ? 'Kedaluwarsa' : ucfirst($sub->status)) }}
                    </span>
                </td>
            </tr>
            @empty
