        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50 transition">
                <td class="px-5 py-3 font-medium text-gray-900">
                    <a href="{{ url('/blog/' . $post->slug) }}" target="_blank" class="hover:text-brand-600">{{ $post->title }}</a>
                </td>
                <td class="px-5 py-3 hidden md:table-cell">
                    @if($post->category)
                    <span class="text-xs font-medium text-brand-600">{{ $post->category->name }}</span>
                    @else
...
function editPost(data) {
    ...
    const f = document.getElementById('postForm');
    f.action = '{{ url('/admin/blog') }}/' + data.id;
