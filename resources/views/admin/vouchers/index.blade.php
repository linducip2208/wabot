...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">Manajemen Voucher</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ $vouchers->count() }} voucher tersedia</p>
    </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Total Voucher</div><div class="text-xl font-extrabold text-gray-900">{{ $vouchers->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        ...
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Aktif</div><div class="text-xl font-extrabold text-gray-900">{{ $active->count() }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center"><i class="fas fa-clock text-amber-500"></i></div>
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Terpakai</div><div class="text-xl font-extrabold text-gray-900">{{ $vouchers->sum('used_count') }}</div></div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center gap-3">
        <div class="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center"><i class="fas fa-ban text-red-400"></i></div>
        <div><div class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Habis</div><div class="text-xl font-extrabold text-gray-900">{{ $vouchers->where('used_count', '>=', 'max_uses')->count() }}</div></div>
    </div>
</div>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                ...
                <th class="px-5 py-3 hidden md:table-cell">Durasi</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50 transition">
                ...
                <td class="px-5 py-3 text-gray-600 hidden md:table-cell text-xs">{{ $v->duration_days }} hari</td>
                <td class="px-5 py-3">
                ...
                <td class="px-5 py-3">
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full {{ $v->is_active && $v->used_count < $v->max_uses ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600' }}">
                        {{ $v->is_active && $v->used_count < $v->max_uses ? 'Aktif' : 'Nonaktif' }}
                    </span>
                </td>
                <td class="px-5 py-3 hidden lg:table-cell text-xs text-gray-400">{{ $v->created_at->format('d M Y H:i') }}</td>
                <td class="px-5 py-3 text-right">
                    <form method="POST" action="{{ route('admin.vouchers.destroy', $v) }}" class="inline" onsubmit="return confirm('Hapus voucher {{ $v->code }}?')">
                        @csrf @method('DELETE')
            ...
            <tr>
                <td colspan="7" class="px-5 py-16 text-center">
                    ...
                    <p class="text-gray-500 font-medium">Belum ada voucher</p>
                    <p class="text-sm text-gray-400">Buat voucher untuk memberikan akses paket ke pengguna</p>
                </td>
            </tr>
            @endforelse
        </tbody>
...
<div id="voucherModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4">Buat Voucher Baru</h2>
        <form method="POST" action="{{ route('admin.vouchers.store') }}" class="space-y-3">
            ...
            <div>
                <label class="text-xs font-medium text-gray-500">Maksimal Pemakaian</label>
                ...
                <p class="text-[11px] text-gray-400 mt-1">Berapa kali voucher ini bisa dipakai</p>
            </div>
            <div>
                <label class="text-xs font-medium text-gray-500">Durasi (Hari)</label>
                ...
                <p class="text-[11px] text-gray-400 mt-1">Masa aktif langganan setelah redeem</p>
            </div>
            <div class="pt-1 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 flex items-start gap-2">
                <i class="fas fa-info-circle mt-0.5 flex-shrink-0"></i>
                <span>Kode voucher 12 karakter akan dibuat otomatis dan bisa dibagikan ke pengguna.</span>
            </div>
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700">Buat Voucher</button>
            </div>
        </form>
    </div>
</div>
