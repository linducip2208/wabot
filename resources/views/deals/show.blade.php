...
<div class="flex items-center justify-between mb-5">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Dibuat {{ $deal->created_at->format('d M Y') }}</p>
        </div>
    </div>
    <a href="{{ route('deals.edit', $deal) }}" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2"><i class="fas fa-edit text-xs"></i> Edit</a>
</div>
    ...
    <div class="lg:col-span-2 space-y-4">
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div><div class="text-xs text-gray-500 mb-1">Nilai Deal</div><div class="text-lg font-extrabold text-emerald-600">Rp {{ number_format($deal->value, 0, ',', '.') }}</div></div>
                ...
                <div><div class="text-xs text-gray-500 mb-1">Status</div><div class="font-medium text-gray-800 capitalize">{{ $deal->status }}</div></div>
                <div><div class="text-xs text-gray-500 mb-1">Target Close</div><div class="font-medium text-gray-800">{{ $deal->expected_close_date?->format('d M Y') ?? '-' }}</div></div>
            </div>
        </div>
        @if($deal->notes)
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            <div class="text-xs font-semibold text-gray-500 mb-2">Catatan</div>
