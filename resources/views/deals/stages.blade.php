...
<div class="flex items-center gap-3 mb-5">
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ $stages->count() }} tahapan pipeline penjualan</p>
    </div>
</div>
...
<div class="grid lg:grid-cols-3 gap-4">
    <div class="lg:col-span-2 space-y-2">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <form method="POST" action="{{ route('deal-stages.destroy', $stage) }}" class="mt-2 flex justify-end" onsubmit="return confirm('Hapus stage?')">
                @csrf @method('DELETE')
                <button class="text-xs text-red-500 hover:text-red-700"><i class="fas fa-trash mr-1"></i> Hapus</button>
            </form>
        </div>
        @empty
        <div class="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
            <i class="fas fa-layer-group text-4xl text-gray-300 mb-3"></i>
            <p class="text-gray-500">Belum ada stage. Tambahkan di panel kanan.</p>
        </div>
        @endforelse
    </div>

    <div class="bg-white rounded-xl border border-gray-200 p-5 h-fit">
        <h2 class="font-bold text-gray-900 mb-3">Tambah Stage</h2>
        <form method="POST" action="{{ route('deal-stages.store') }}" class="space-y-3">
            @csrf
            <div>
                <label class="text-xs font-medium text-gray-500">Nama Stage</label>
            ...
            <button type="submit" class="w-full bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700"><i class="fas fa-plus mr-1"></i> Tambah Stage</button>
        </form>
    </div>
</div>
@endsection
