@extends('layouts.app')
@section('title', __('deals.new_deal') . ' — WABot')
@section('content')
...
<div class="flex items-center gap-3 mb-5">
    ...
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">{{ __('deals.new_deal') }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ __('common.create') }} {{ __('deals.deal') }}</p>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-lg">
    <form method="POST" action="{{ route('deals.store') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div>
                ...
                <select name="stage_id" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                    <option value="">{{ __('common.select') }} {{ __('deals.stage') }}...</option>
        ...
        <div class="flex gap-2 pt-1">
            <a href="{{ route('deals.index') }}" class="flex-1 text-center bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">{{ __('common.cancel') }}</a>
