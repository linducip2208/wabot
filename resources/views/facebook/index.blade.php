@extends('layouts.app')
@section('title', __('facebook.title') . ' — WABot')

@section('content')
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900">{{ __('facebook.title') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('facebook.subtitle') }}</p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            ...
            <i class="fas fa-plus text-xs"></i> {{ __('facebook.create_account') }}
        </button>
    </div>
    ...
    @if($accounts->isEmpty())
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1">{{ __('facebook.empty_title') }}</h3>
            <p class="text-sm text-gray-400 mb-4">{{ __('facebook.empty_desc') }}</p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                class="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 transition">
                {{ __('facebook.create_account') }}
            </button>
        </div>
    @else
        <div class="space-y-3">
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                @if($acc->page_name)
                                    <p class="text-xs text-gray-400 mt-0.5">{{ $acc->page_name }}</p>
                                @endif
                                <p class="text-xs text-gray-400 mt-0.5">Page ID: {{ $acc->page_id }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-1.5">
                            @if($acc->status !== 'connected')
                                <form action="{{ route('facebook.connect', $acc) }}" method="POST">
                                    @csrf
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition">
                                        <i class="fab fa-facebook mr-1"></i>{{ __('common.connect') }}
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('facebook.disconnect', $acc) }}" method="POST">
                                    @csrf
                            ...
                            @endif
                            <form action="{{ route('facebook.destroy', $acc) }}" method="POST" onsubmit="return confirm('{{ __('common.delete') }}?')" class="inline">
                                @csrf @method('DELETE')
    ...
    <div class="mt-4 bg-blue-50 rounded-xl border border-blue-100 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1">{{ __('facebook.webhook_url') }}</h3>
                <p class="text-xs text-gray-500 mb-2">{{ __('facebook.webhook_hint') }}</p>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('facebook.create_account') }}</h2>
        <form action="{{ route('facebook.store') }}" method="POST" class="space-y-3">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('facebook.page_id') }}</label>
                <input type="text" name="page_id" placeholder="Dari Meta Developer" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('facebook.page_token') }}</label>
                <input type="text" name="page_token" placeholder="Page Access Token" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('facebook.app_secret_label') }}</label>
