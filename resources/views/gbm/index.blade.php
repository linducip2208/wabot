@extends('layouts.app')
@section('title', 'Google Business Messages — WABot')
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                @if($acc->brand_id)
                                    <p class="text-xs text-gray-400 mt-0.5">Brand: {{ $acc->brand_id }}</p>
                                @endif
                                @if($acc->agent_id)
                                    <p class="text-xs text-gray-400">Agent: {{ $acc->agent_id }}</p>
                                @endif
                            </div>
                        </div>
                        <div class="flex items-center gap-1.5">
                            @if(!$acc->is_active)
                                <form action="{{ route('gbm.connect', $acc) }}" method="POST">
                                    @csrf
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition">
                                        <i class="fab fa-google mr-1"></i>{{ __('common.connect') }}
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('gbm.disconnect', $acc) }}" method="POST">
                                    @csrf
                            ...
                            @endif
                            <form action="{{ route('gbm.destroy', $acc) }}" method="POST" onsubmit="return confirm('{{ __('common.delete') }}?')" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">
                    ...
                    4. Webhook URL: <code class="bg-blue-100 px-1 rounded">{{ route('webhook.gbm') }}</code>
                </p>
            </div>
        </div>
    </div>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        ...
        <form action="{{ route('gbm.store') }}" method="POST" class="space-y-3">
            @csrf
            <div>
