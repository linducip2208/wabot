@extends('layouts.app')
@section('title', __('meta.title') . ' — WABot')

@section('content')
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900">{{ __('meta.title') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('meta.subtitle') }}</p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            ...
            <i class="fas fa-plus text-xs"></i> {{ __('meta.create_account') }}
        </button>
    </div>
    ...
    @if($accounts->isEmpty())
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1">{{ __('meta.empty_title') }}</h3>
            <p class="text-sm text-gray-400 mb-4">{{ __('meta.empty_desc') }}</p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-5">
                    <div class="flex items-start justify-between flex-wrap gap-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        ...
                                        {{ $account->status === 'connected' ? __('common.connected') : __('common.disconnected') }}
                                    </span>
                                </div>
                                <div class="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
                                    @if($account->phone_number)
                                        <span><i class="fas fa-phone mr-1"></i>{{ $account->phone_number }}</span>
                                    @endif
                                    @if($account->waba_id)
                                        <span><i class="fas fa-building mr-1"></i>{{ Str::limit($account->waba_id, 18) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-1.5">
                            @if($account->status !== 'connected')
                                <form action="{{ route('meta.connect', $account) }}" method="POST">
                                    @csrf
                            ...
                            @else
                                <form action="{{ route('meta.disconnect', $account) }}" method="POST">
                                    @csrf
                            ...
                            @endif
                            <button onclick="openTestModal({{ $account->id }})"
                            ...
                            <form action="{{ route('meta.destroy', $account) }}" method="POST" onsubmit="return confirm('{{ __('meta.delete_confirm') }}')" class="inline">
                                @csrf @method('DELETE')
                    ...
                    @if($account->status === 'connected')
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            @php $metaSessions = $account->sessions ?? collect(); @endphp
                            @if($metaSessions->isNotEmpty())
                                <div class="flex flex-wrap gap-1.5 mb-3">
                                    @foreach($metaSessions as $s)
                            ...
                            @endif
                            <form action="{{ route('meta.session.store', $account) }}" method="POST" class="flex items-end gap-3">
                                @csrf
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.create_session') }}</label>
                                    <input type="text" name="name" placeholder="{{ __('common.name') }} {{ __('common.session') }}" required
    ...
    <div class="mt-4 bg-brand-50 rounded-xl border border-brand-200 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1">{{ __('meta.webhook_url') }}</h3>
                <p class="text-xs text-gray-500 mb-2">{{ __('meta.webhook_hint') }}</p>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('meta.create_account') }}</h2>
        <form action="{{ route('meta.store') }}" method="POST" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.account_name') }}</label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.phone_number_id') }}</label>
                <input type="text" name="phone_number_id" placeholder="Dari Meta Developer Dashboard" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.access_token') }}</label>
                <input type="text" name="access_token" placeholder="Permanent access token" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.app_id') }} <span class="text-gray-400">({{ __('common.optional') }})</span></label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.verify_token') }} <span class="text-gray-400">({{ __('common.optional') }})</span></label>
...
{{-- {{ __('common.edit') }} & TEST MODALS --}}
@foreach($accounts as $account)
<div id="editModal{{ $account->id }}" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('meta.edit_account') }}: {{ $account->name }}</h2>
        <form action="{{ route('meta.update', $account) }}" method="POST" class="space-y-3">
            @csrf @method('PUT')
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.phone_number_id') }}</label>
                <input type="text" name="phone_number_id" value="{{ $account->phone_number_id }}" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.access_token') }}</label>
                <input type="text" name="access_token" value="{{ $account->access_token }}" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('editModal{{ $account->id }}').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">{{ __('common.cancel') }}</button>
                <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700">{{ __('common.update') }}</button>
            </div>
        </form>
    </div>
</div>

<div id="testModal{{ $account->id }}" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('meta.test_send') }}: {{ $account->name }}</h2>
        <form action="{{ route('meta.test', $account) }}" method="POST" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('meta.target_number') }}</label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('testModal{{ $account->id }}').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">{{ __('common.cancel') }}</button>
                <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700">{{ __('common.send') }}</button>
            </div>
        </form>
    </div>
</div>
