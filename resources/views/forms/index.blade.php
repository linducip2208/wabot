@extends('layouts.app')
@section('title', __('forms.title') . ' — WABot')

@section('content')
<div class="max-w-6xl mx-auto" x-data="formBuilder()">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900">{{ __('forms.title') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('forms.subtitle') }}</p>
        </div>
        <button @click="showBuilder = true"
            ...
            <i class="fas fa-plus text-xs"></i> {{ __('forms.create_button') }}
        </button>
    </div>
    ...
    @if($forms->isEmpty())
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1">{{ __('forms.empty_title') }}</h3>
            <p class="text-sm text-gray-400 mb-4">{{ __('forms.empty_desc') }}</p>
            <button @click="showBuilder = true"
                class="bg-brand-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition">
                {{ __('forms.create_first') }}
            </button>
        </div>
    @else
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-5 hover:border-brand-300 hover:shadow-sm transition">
                    <div class="flex items-start justify-between mb-3">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">{{ $form->metaAccount?->name ?? __('forms.no_meta_account') }}</p>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            {{ $form->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
                            {{ $form->status === 'active' ? __('common.active') : __('common.draft') }}
                        </span>
                    </div>
                    <div class="flex items-center gap-4 text-xs text-gray-400 mb-3">
                        <span><i class="fas fa-cube mr-1"></i> {{ __('forms.fields_count', ['count' => count($form->components ?? [])]) }}</span>
                        <span><i class="fas fa-envelope mr-1"></i> {{ __('forms.submission_count', ['count' => $form->submission_count]) }}</span>
                    </div>
                    <div class="flex items-center gap-1.5 flex-wrap">
                        ...
                        <button onclick="openSendModal({{ $form->id }})"
                            class="px-3 py-1.5 rounded-lg text-[11px] font-medium bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition">
                            <i class="fas fa-paper-plane mr-1"></i>{{ __('common.send') }}
                        </button>
                        <form action="{{ route('forms.destroy', $form) }}" method="POST" onsubmit="return confirm('{{ __('common.delete') }}?')" class="inline">
                            @csrf @method('DELETE')
    ...
    <div x-show="showBuilder" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" x-cloak
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto" @click.stop="">
            <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('forms.builder_title') }}</h2>
            <form :action="editingId ? '/forms/' + editingId : '{{ route('forms.store') }}'" method="POST" class="space-y-3">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('forms.meta_account') }}</label>
                        <select name="meta_account_id" x-model="formMetaId"
                            class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                            <option value="">-- {{ __('common.select') }} --</option>
                            @foreach(\App\Models\WaMetaAccount::where('user_id', Auth::id())->where('status','connected')->get() as $acc)
                    ...
                    <div class="space-y-1.5 max-h-60 overflow-y-auto rounded-xl border border-gray-200 p-3 bg-gray-50">
                        <template x-for="(field, i) in fields" :key="i">
                            <div class="flex items-center gap-2 p-2 bg-white rounded-lg border border-gray-200 text-xs">
                                ...
                                <input type="text" x-model="field.label" placeholder="{{ __('forms.label') }}" class="border border-gray-200 rounded-lg px-2 py-1.5 text-xs flex-1">
                                <input type="text" x-model="field.placeholder" placeholder="{{ __('forms.placeholder') }}" class="border border-gray-200 rounded-lg px-2 py-1.5 text-xs w-24"
                                ...
                                <label class="flex items-center gap-1 text-gray-500 whitespace-nowrap">
                                    <input type="checkbox" x-model="field.required"> {{ __('forms.required') }}
                                </label>
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-brand-700 transition">
                        <span x-text="editingId ? '{{ __('forms.update') }}' : '{{ __('common.save') }}'"></span>
                    </button>
                </div>
            </form>
        </div>
...
{{-- SEND MODALS --}}
@foreach($forms as $form)
<div id="sendModal{{ $form->id }}" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4">{{ __('common.send') }}: {{ $form->name }}</h2>
        <form action="{{ route('forms.send', $form) }}" method="POST" class="space-y-3">
            @csrf
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('forms.meta_account') }}</label>
                <select name="meta_account_id" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                    @foreach(\App\Models\WaMetaAccount::where('user_id', Auth::id())->where('status','connected')->get() as $acc)
                        <option value="{{ $acc->id }}" {{ $form->meta_account_id == $acc->id ? 'selected' : '' }}>{{ $acc->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('forms.target_number') }}</label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('sendModal{{ $form->id }}').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">{{ __('common.cancel') }}</button>
