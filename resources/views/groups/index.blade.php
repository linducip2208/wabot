@extends('layouts.app')
@section('title', 'Grup Kontak — WABot')
@section('content')

<div class="flex items-center justify-between mb-5">
    <div><h1 class="text-xl font-extrabold text-gray-900">Grup Kontak</h1><p class="text-sm text-gray-500 mt-0.5">Kelompokkan kontak untuk campaign</p></div>
    <button onclick="toggleModal()" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2"><i class="fas fa-plus text-xs"></i> Tambah Grup</button>
</div>
...
<div class="grid gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500">{{ $g->contacts_count }} kontak</div>
            </div>
        </div>
        <div class="flex items-center gap-1">
            ...
            <form method="POST" action="{{ route('groups.destroy', $g) }}" onsubmit="return confirm('Hapus grup?')">
                @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500">Belum ada grup kontak</p>
    </div>
    @endforelse
</div>
...
<div id="groupModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="gTitle">Tambah Grup</h2>
        <form method="POST" action="{{ route('groups.store') }}" class="space-y-3" id="gForm">
            @csrf
            <div id="gMethod"></div>
            <input type="text" name="name" placeholder="Nama grup" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            ...
            <div class="flex gap-2">
                ...
                <button type="submit" class="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
function toggleModal() {
    ...
    if (!m.classList.contains('hidden')) { document.getElementById('gTitle').textContent='Tambah Grup'; document.getElementById('gForm').reset(); document.getElementById('gMethod').innerHTML=''; }
}
function editGroup(id, name, color) {
    const m = document.getElementById('groupModal'); m.classList.remove('hidden');
    document.getElementById('gTitle').textContent='Edit Grup';
