@extends('layouts.app')
@section('title', 'Kirim Pesan — WABot')
@section('content')

<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-5">
        <a href="{{ route('messages.sent') }}" class="text-sm text-gray-500 hover:text-brand-600"><i class="fas fa-arrow-left mr-1"></i> Kembali</a>
        <span class="text-gray-300">|</span>
        <a href="{{ route('messages.received') }}" class="text-xs text-gray-500 hover:text-brand-600 {{ request()->is('messages/received') ? 'font-semibold text-brand-600' : '' }}">Masuk</a>
        <a href="{{ route('messages.sent') }}" class="text-xs text-gray-500 hover:text-brand-600 {{ request()->is('messages/sent') ? 'font-semibold text-brand-600' : '' }}">Terkirim</a>
        <a href="{{ route('messages.queue') }}" class="text-xs text-gray-500 hover:text-brand-600 {{ request()->is('messages/queue') ? 'font-semibold text-brand-600' : '' }}">Antrian</a>
    </div>

    <h1 class="text-xl font-extrabold text-gray-900 mb-1">Kirim Pesan</h1>
    <p class="text-sm text-gray-500 mb-5">Kirim WhatsApp langsung ke nomor atau kontak</p>
    ...
    <form method="POST" action="{{ route('messages.send') }}" class="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
        ...
        <div>
            <label class="text-xs font-medium text-gray-500">Tujuan</label>
            <div class="flex gap-2 mb-2">
                <button type="button" onclick="switchTab('manual')" id="tabManual" class="text-xs px-3 py-1.5 rounded-lg bg-brand-600 text-white font-medium">Nomor Manual</button>
                <button type="button" onclick="switchTab('contact')" id="tabContact" class="text-xs px-3 py-1.5 rounded-lg bg-gray-100 text-gray-700 font-medium">Dari Kontak</button>
            </div>
            <div id="panelManual">
        ...
        <div>
            <label class="text-xs font-medium text-gray-500">Pesan <span class="text-gray-400">({'Halo|Hai'} = spintax)</span></label>
            <textarea name="message" rows="4" required placeholder="Ketik pesan Anda..."
