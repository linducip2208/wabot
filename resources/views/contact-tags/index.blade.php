@extends('layouts.app')
@section('title', 'Contact Tags — WABot')
@section('content')

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">Contact Tags</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ $tags->count() }} tag · kelompokkan kontak untuk targeting</p>
    </div>
    <button onclick="openModal()" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2">
        <i class="fas fa-plus text-xs"></i> Tag Baru
    </button>
</div>
...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-400">{{ $tag->contacts_count }} kontak</div>
                </div>
            </div>
            <div class="flex items-center gap-1">
                ...
                <form method="POST" action="{{ route('contact-tags.destroy', $tag) }}" onsubmit="return confirm('Hapus tag?')">@csrf @method('DELETE')<button class="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600"><i class="fas fa-trash text-xs"></i></button></form>
            </div>
        </div>
    </div>
    @empty
    <div class="col-span-full bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 font-medium mb-1">Belum ada tag</p>
        <p class="text-sm text-gray-400 mb-4">Buat tag untuk mengelompokkan kontak</p>
        <button onclick="openModal()" class="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition"><i class="fas fa-plus text-xs"></i> Tag Baru</button>
    </div>
    @endforelse
</div>
...
<div id="tagModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="tagModalTitle">Tag Baru</h2>
        <form method="POST" action="{{ route('contact-tags.store') }}" class="space-y-3" id="tagForm">
            @csrf
            <div id="tagMethod"></div>
            <div>
                <label class="text-xs font-medium text-gray-500">Nama Tag</label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('tagModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium">Batal</button>
...
function openModal() {
    ...
    document.getElementById('tagModalTitle').textContent = 'Tag Baru';
