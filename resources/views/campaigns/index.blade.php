@extends('layouts.app')
@section('title', 'Kampanye — WABot')
@section('content')

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900">Kampanye</h1>
        <p class="text-sm text-gray-500 mt-0.5">Bulk messaging ke banyak kontak sekaligus</p>
    </div>
    <a href="{{ route('campaigns.create') }}" class="bg-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-brand-700 transition flex items-center gap-2">
        <i class="fas fa-plus text-xs"></i> Kampanye Baru
    </a>
</div>
...
<div class="grid gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
        ...
        <p class="text-gray-500 mb-1">Belum ada kampanye</p>
        <p class="text-sm text-gray-400">Buat kampanye bulk messaging dengan 3 langkah mudah</p>
        <a href="{{ route('campaigns.create') }}" class="inline-block mt-4 bg-brand-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-brand-700 transition">Buat Kampanye</a>
    </div>
    @endforelse
</div>
@endsection
