            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500">
                        ...
                        · {{ $c->session?->name ?? $c->metaAccount?->name ?? $c->telegramAccount?->name ?? '-' }}@if(count($c->session_ids ?? []) > 1) +{{ count($c->session_ids) - 1 }} ({{ $c->session_strategy === 'random' ? 'Random' : 'Round Robin' }})@endif · {{ $c->delay_min_seconds ?? $c->delay_seconds ?? 300 }}–{{ $c->delay_max_seconds ?? 400 }}s interval</div>
                </div>
            </div>
