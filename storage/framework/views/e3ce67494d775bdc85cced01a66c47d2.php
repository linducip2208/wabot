<?php $__env->startSection('title', __('telegram.title') . ' — WABot'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900"><?php echo e(__('telegram.title')); ?></h1>
            <p class="text-sm text-gray-500 mt-0.5"><?php echo e(__('telegram.subtitle')); ?></p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            class="bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-sky-700 transition flex items-center gap-2">
            <i class="fas fa-plus text-xs"></i> <?php echo e(__('telegram.create_bot')); ?>
    ...
    <?php if($accounts->isEmpty()): ?>
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            <div class="w-14 h-14 mx-auto bg-sky-50 rounded-full flex items-center justify-center mb-4">
                <i class="fab fa-telegram text-sky-500 text-xl"></i>
            </div>
            <h3 class="text-gray-500 font-medium mb-1"><?php echo e(__('telegram.empty_title')); ?></h3>
            <p class="text-sm text-gray-400 mb-4"><?php echo e(__('telegram.empty_desc')); ?></p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                class="bg-sky-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-sky-700 transition">
                <?php echo e(__('telegram.create_bot')); ?>
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <?php if($acc->bot_username): ?>
                                    <p class="text-xs text-gray-400 mt-0.5">{{ $acc->bot_username }}</p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <?php if($acc->status !== 'connected'): ?>
                                <form action="<?php echo e(route('telegram.connect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-sky-600 text-white hover:bg-sky-700 transition">
                                        <i class="fab fa-telegram mr-1"></i><?php echo e(__('common.connect')); ?>

                                    </button>
                                </form>
                            <?php else: ?>
                                <form action="<?php echo e(route('telegram.disconnect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                            ...
                            <form action="<?php echo e(route('telegram.destroy', $acc)); ?>" method="POST" onsubmit="return confirm('<?php echo e(__('common.delete')); ?>?')" class="inline">
    ...
    <div class="mt-4 bg-sky-50 rounded-xl border border-sky-100 p-5">
        <div class="flex items-start gap-4">
            <div class="w-9 h-9 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0">
                <i class="fas fa-info text-sky-600 text-sm"></i>
            </div>
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1"><?php echo e(__('telegram.setup_title')); ?></h3>
                <p class="text-xs text-gray-500"><?php echo __('telegram.setup_desc'); ?></p>
            </div>
        </div>
    </div>
</div>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4"><?php echo e(__('telegram.create_bot')); ?></h2>
        <form action="<?php echo e(route('telegram.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('telegram.bot_token')); ?></label>
                <input type="text" name="bot_token" placeholder="Dari @BotFather" required
            ...
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-sky-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-sky-700"><?php echo e(__('common.save')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="testModal<?php echo e($acc->id); ?>" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        ...
        <form action="<?php echo e(route('telegram.test', $acc)); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('telegram.chat_id')); ?></label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('testModal<?php echo e($acc->id); ?>').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium"><?php echo e(__('common.cancel')); ?></button>
                <button type="submit" class="flex-1 bg-sky-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-sky-700"><?php echo e(__('common.send')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\telegram\index.blade.php ENDPATH**/ ?>