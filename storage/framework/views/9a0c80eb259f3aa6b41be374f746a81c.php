  ...
  <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
    ...
    <div class="p-7 space-y-5 fade-up">
      ...
      <div class="bg-slate-50 border border-slate-200 rounded-xl p-5">
        <div class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2"><?php echo e(__('common.product')); ?></div>
        <div class="flex items-baseline gap-3">
          <span class="text-2xl">📦</span>
          <div>
            <div class="text-lg font-bold text-slate-900"><?php echo e($data['product']['name'] ?? 'Aplikasi'); ?></div>
            <div class="text-sm text-slate-500">Versi v<?php echo e($data['product']['version'] ?? '1.0.0'); ?></div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
          ...
          <div class="font-mono text-sm text-slate-800 break-all"><?php echo e($data['domain'] ?? '-'); ?></div>
        </div>

        <?php if(!empty($data['license']['support_until'])): ?>
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
          <div class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 flex items-center gap-1.5">
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            Support <?php echo e(__('common.active')); ?> Sampai
          </div>
          <div class="font-medium text-sm text-slate-800"><?php echo e(\Illuminate\Support\Carbon::parse($data['license']['support_until'])->format('d M Y')); ?></div>
        </div>
        <?php endif; ?>
      </div>
      ...
      <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div class="text-sm text-emerald-800 leading-relaxed">
          <strong>Konfirmasi:</strong> <?php echo e(__('common.name')); ?> <?php echo e(__('common.product')); ?> di atas harus cocok dengan yang kamu beli. Kalau salah, klik "Revoke" di marketplace dan re-pair dengan key yang benar.
        </div>
      </div>
...
<?php /**PATH D:\project laravel\wabot\resources\views\license\pair-success.blade.php ENDPATH**/ ?>