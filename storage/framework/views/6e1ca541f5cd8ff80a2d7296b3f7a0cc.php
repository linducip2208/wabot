<?php $__env->startSection('title', __('admin.credit_packs') . ' — Admin'); ?>
<?php $__env->startSection('content'); ?>

<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-gray-900"><?php echo e(__('admin.credit_packs')); ?></h1>
        <p class="text-sm text-gray-500 mt-0.5"><?php echo e(__('admin.credit_packs_count', ['count' => $packs->count()])); ?></p>
    </div>
    <button onclick="toggleModal()"
        ...
        <i class="fas fa-plus text-xs"></i> <?php echo e(__('admin.create_pack')); ?>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                ...
                <th class="px-5 py-3"><?php echo e(__('credits.credits')); ?></th>
                <th class="px-5 py-3"><?php echo e(__('common.price')); ?></th>
                <th class="px-5 py-3"><?php echo e(__('common.status')); ?></th>
                <th class="px-5 py-3"><?php echo e(__('common.order')); ?></th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50/50">
                <td class="px-5 py-3 font-medium text-gray-900"><?php echo e($pack->name); ?></td>
                <td class="px-5 py-3 font-semibold text-gray-900"><?php echo e(number_format($pack->credits)); ?></td>
                <td class="px-5 py-3 text-gray-600">Rp <?php echo e(number_format($pack->price, 0, ',', '.')); ?></td>
                <td class="px-5 py-3">
                    <form method="POST" action="<?php echo e(route('admin.credit-packs.toggle', $pack)); ?>" class="inline">
                        <?php echo csrf_field(); ?>
                        <button class="text-xs font-medium px-2 py-0.5 rounded-full <?php echo e($pack->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'); ?>">
                            <?php echo e($pack->is_active ? __('common.active') : __('common.inactive')); ?>

                        </button>
                    </form>
                </td>
                <td class="px-5 py-3 text-gray-600"><?php echo e($pack->sort_order); ?></td>
                <td class="px-5 py-3 text-right">
                    <form method="POST" action="<?php echo e(route('admin.credit-packs.destroy', $pack)); ?>" class="inline" onsubmit="return confirm('<?php echo e(__('common.delete')); ?>?')">
            ...
            <tr><td colspan="6" class="px-5 py-16 text-center text-gray-500"><?php echo e(__('admin.no_credit_packs')); ?></td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
...
<div id="packModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4"><?php echo e(__('admin.create_pack')); ?></h2>
        <form method="POST" action="<?php echo e(route('admin.credit-packs.store')); ?>" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500"><?php echo e(__('credits.credits')); ?></label>
            ...
            <div>
                <label class="text-xs font-medium text-gray-500"><?php echo e(__('common.price')); ?> (Rp)</label>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\admin\credit-packs\index.blade.php ENDPATH**/ ?>