<?php $__env->startSection('title', 'Chat — WABot'); ?>

<?php $__env->startSection('chat_content'); ?>
<div x-data="chatApp(<?php echo e($activeContact?->id ?? 'null'); ?>, <?php echo e($activeContact?->phone ? "'{$activeContact->phone}'" : 'null'); ?>, <?php echo e($socketServerUrl ? "'{$socketServerUrl}'" : 'null'); ?>, <?php echo e($socketApiKey ? "'{$socketApiKey}'" : 'null'); ?>)" class="flex h-full"
    x-init="sessionsData = <?php echo e(json_encode($sessions->map(fn($s) => ['id' => $s->id, 'session_id' => $s->session_id, 'name' => $s->name, 'phone' => $s->phone, 'channel' => $s->channel ?? 'whatsapp', 'meta_account_id' => $s->meta_account_id]))); ?>;
    contactsData = <?php echo e(json_encode($contacts->map(fn($c) => ['id' => $c->id, 'name' => $c->name, 'phone' => $c->phone, 'display_phone' => $c->display_phone, 'is_lid' => str_contains($c->phone, '@lid'), 'last_message' => $c->last_message, 'last_time' => $c->last_time?->format('H:i'), 'last_direction' => $c->last_direction, 'last_session_id' => $c->last_session_id, 'channel' => $c->channel ?? 'whatsapp']))); ?>;
    instagramAccountsData = <?php echo e(json_encode($instagramAccounts->map(fn($a) => ['id' => 'ig_'.$a->id, 'name' => $a->name]))); ?>;
    telegramAccountsData = <?php echo e(json_encode($telegramAccounts->map(fn($a) => ['id' => 'tg_'.$a->id, 'name' => $a->name]))); ?>;
    metaAccountsData = <?php echo e(json_encode($metaAccounts->map(fn($a) => ['id' => $a->id, 'name' => $a->name]))); ?>;
    ">
    ...
    <aside class="w-full md:w-80 lg:w-96 flex-shrink-0 bg-gray-900 flex flex-col border-r border-gray-800"
        ...
        <div class="p-4 border-b border-gray-800">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-400"><?php echo e($sessions->count()); ?> <?php echo e(__('common.online')); ?></span>
                </div>
            </div>
            ...
            <div class="flex gap-1 mb-3 overflow-x-auto pb-1" style="scrollbar-width:thin">
                ...
                <button @click="activeTab = <?php echo e($s->id); ?>"
                    class="flex-shrink-0 text-[11px] px-2.5 py-1.5 rounded-lg font-medium transition flex items-center gap-1"
                    :class="activeTab === <?php echo e($s->id); ?> ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                    <?php echo e(\Str::limit($s->name, 10)); ?>
            ...
            <div class="relative">
                <input type="text" placeholder="<?php echo e(__('common.search')); ?> <?php echo e(__('common.contact')); ?>..." x-model="searchQuery"
        ...
        <nav class="flex-1 overflow-y-auto" id="contactList">
                ...
                <div @click="openChat(c)"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span @click.stop="openChat(c); $nextTick(() => showEditModal = true)" x-show="!c.display_phone && !c.is_lid && c.channel !== 'instagram' && c.channel !== 'telegram'" class="text-[11px] text-orange-400 cursor-pointer hover:underline">+ <?php echo e(__('chat.set_phone_number')); ?></span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-xs text-gray-400 truncate" x-text="c.last_message || '<?php echo e(__('chat.no_messages_yet')); ?>'"></span>
                        </div>
                    </div>
                </div>
            </template>
            <div x-show="filteredContacts.length === 0" class="px-4 py-16 text-center">
                ...
                <p class="text-gray-500 text-sm"><?php echo e(__('chat.no_conversations')); ?></p>
                <p class="text-gray-600 text-xs mt-1"><?php echo e(__('chat.messages_will_appear')); ?></p>
            </div>
        </nav>
    ...
    <main class="flex-1 flex flex-col bg-[#efeae2]"
        ...
        <div x-show="!activeContact" class="flex-1 flex items-center justify-center">
            <div class="text-center">
                ...
                <p class="text-gray-500 text-sm max-w-xs"><?php echo e(__('common.select')); ?> <?php echo e(__('common.contact')); ?> dari sidebar untuk mulai chat.<br>Auto-reply <?php echo e(__('common.active')); ?> untuk keyword yang cocok.</p>
            </div>
        </div>
            ...
            <div class="flex-1 flex flex-col h-full">
                ...
                <div class="flex items-center gap-3 bg-[#f0f2f5] px-4 py-2.5 border-b border-gray-200">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            ...
                            <button @click.stop="showEditModal = true" class="p-0.5 rounded hover:bg-gray-200 transition flex-shrink-0" title="<?php echo e(__('common.edit')); ?> <?php echo e(__('common.contact')); ?>">
                        ...
                        <span x-show="!activeContact.display_phone && !activeContact.is_lid && activeContact.channel !== 'instagram' && activeContact.channel !== 'telegram'" @click="showEditModal = true" class="text-xs text-orange-500 cursor-pointer hover:underline">+ <?php echo e(__('chat.set_phone_number')); ?></span>
                        <div class="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
                            <span x-show="sessionId && isSessionChannel"><?php echo e(__('chat.online_via')); ?> <span x-text="sessionName"></span></span>
                            <span x-show="!sessionId && isSessionChannel" class="text-red-500"><?php echo e(__('chat.no_session_connected')); ?></span>
                            ...
                            <template x-if="autoreplies.length > 0">
                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-purple-100 text-purple-700">
                                    ...
                                    <span x-text="autoreplies.length"></span> auto-reply <?php echo e(__('common.active')); ?>
                ...
                <div class="flex-1 overflow-y-auto px-4 md:px-12 py-4 space-y-1.5" id="messageArea"
                            ...
                            <div class="max-w-[75%] md:max-w-[60%]">
                                ...
                                <div class="flex items-center gap-1.5 mt-0.5"
                                    ...
                                    <template x-if="msg.direction === 'out'">
                                        <svg class="w-3 h-3" :class="msg.<?php echo e(__('common.status')); ?> === 'sent' ? 'text-gray-400' : 'text-blue-500'" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div x-show="messages.length === 0" class="flex items-center justify-center h-full">
                        <p class="text-gray-500 text-sm"><?php echo e(__('chat.no_messages_in_chat')); ?></p>
                    </div>
                </div>
                ...
                <div class="bg-[#f0f2f5] px-4 py-3 flex items-end gap-3 border-t border-gray-200">
                    <textarea x-model="newMessage" @keydown.enter.prevent="!$event.shiftKey && sendMessage()"
                        placeholder="<?php echo e(__('chat.type_message_placeholder')); ?>"
                        rows="1"
...
<div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
    ...
    <div class="relative bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" @click.stop="">
        <h3 class="text-lg font-bold text-gray-900 mb-4"><?php echo e(__('common.edit')); ?> <?php echo e(__('common.contact')); ?></h3>
        <div class="space-y-3">
            <div>
            ...
            <div>
                    <label class="text-xs font-medium text-gray-500"><?php echo e(__('chat.phone_display_label')); ?></label>
                <input type="text" x-model="editPhone" placeholder="628xxxx" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                    <p class="text-[10px] text-gray-400 mt-1"><?php echo e(__('chat.phone_display_hint')); ?></p>
            </div>
        </div>
        <div class="flex gap-2 mt-4">
            <button @click="showEditModal = false" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2 text-sm font-medium hover:bg-gray-200"><?php echo e(__('common.cancel')); ?></button>
            <button @click="saveContact()" class="flex-1 bg-brand-600 text-white rounded-xl py-2 text-sm font-semibold hover:bg-brand-700"><?php echo e(__('common.save')); ?></button>
        </div>
    </div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.chat', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\chat\index.blade.php ENDPATH**/ ?>