<?php $__env->startSection('title', __('line.title') . ' — WABot'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900"><?php echo e(__('line.title')); ?></h1>
            <p class="text-sm text-gray-500 mt-0.5"><?php echo e(__('line.subtitle')); ?></p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            ...
            <i class="fas fa-plus text-xs"></i> <?php echo e(__('line.add_channel')); ?>
    ...
    <?php if($accounts->isEmpty()): ?>
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1"><?php echo e(__('line.empty_title')); ?></h3>
            <p class="text-sm text-gray-400 mb-4"><?php echo e(__('line.empty_desc')); ?></p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                class="bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-green-700 transition">
                <?php echo e(__('line.add_channel')); ?>
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <?php if($acc->display_name): ?>
                                    <p class="text-xs text-gray-400 mt-0.5"><?php echo e($acc->display_name); ?></p>
                                <?php endif; ?>
                                <?php if($acc->channel_id): ?>
                                    <p class="text-[10px] text-gray-400 font-mono">Channel ID: <?php echo e($acc->channel_id); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <?php if(!$acc->is_active): ?>
                                <form action="<?php echo e(route('line.connect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-green-600 text-white hover:bg-green-700 transition">
                                        <i class="fab fa-line mr-1"></i><?php echo e(__('common.connect')); ?>

                                    </button>
                                </form>
                            <?php else: ?>
                                <form action="<?php echo e(route('line.disconnect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                            ...
                            <?php endif; ?>
                            <form action="<?php echo e(route('line.destroy', $acc)); ?>" method="POST" onsubmit="return confirm('<?php echo e(__('common.delete')); ?>?')" class="inline">
    ...
    <div class="mt-4 bg-green-50 rounded-xl border border-green-100 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1"><?php echo e(__('line.setup_title')); ?></h3>
                <p class="text-xs text-gray-500 mb-2"><?php echo __('line.setup_desc'); ?></p>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4"><?php echo e(__('line.add_channel')); ?></h2>
        <form action="<?php echo e(route('line.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('line.channel_id')); ?></label>
                <input type="text" name="channel_id" placeholder="From LINE Developers Console" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('line.channel_secret')); ?></label>
                <input type="text" name="channel_secret" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('line.channel_access_token')); ?></label>
            ...
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-green-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-green-700"><?php echo e(__('common.save')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="testModal<?php echo e($acc->id); ?>" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        ...
        <form action="<?php echo e(route('line.test', $acc)); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('line.user_id')); ?></label>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('testModal<?php echo e($acc->id); ?>').classList.add('hidden')"
                    class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium"><?php echo e(__('common.cancel')); ?></button>
                <button type="submit" class="flex-1 bg-green-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-green-700"><?php echo e(__('common.send')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\line\index.blade.php ENDPATH**/ ?>