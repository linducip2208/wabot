...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6" x-data="widgetBuilder()">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-3 p-3 border border-gray-200 rounded-xl hover:border-indigo-300 transition cursor-pointer"
                                    ...
                                    <select x-show="hasChannel('<?php echo e($ch['type']); ?>')"
                                        ...
                                        <option value="<?php echo e(is_array($acc) ? ($acc['id'] ?? '') : $acc); ?>"><?php echo e(is_array($acc) ? ($acc['name'] ?? ($acc['bot_username'] ?? ($acc['username'] ?? ($acc['page_id'] ?? ($acc['phone_number'] ?? ''))))) : $acc); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </select>
                                    <?php else: ?>
    ...
    <?php $__currentLoopData = $widgets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $widget): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <?php $wdata = $widget->only(['name', 'greeting_message', 'offline_message', 'theme_color', 'position', 'button_icon', 'channels', 'is_active']); ?>
    <script type="application/json" id="widget-data-<?php echo e($widget->id); ?>"><?php echo json_encode($wdata, 15, 512) ?></script>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>
