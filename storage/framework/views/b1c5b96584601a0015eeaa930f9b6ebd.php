<?php $__env->startSection('title', 'Button Baru — WABot'); ?>
...
<div class="flex items-center gap-3 mb-5">
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5"><?php echo e(__('common.message')); ?> dengan tombol pilihan</p>
    </div>
</div>

<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-xl" x-data="btnForm(<?php echo e(old('buttons') ? old('buttons') : '[]'); ?>)">
    <form method="POST" action="<?php echo e(route('buttons.store')); ?>" class="space-y-4" @submit="syncButtons">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <select name="header_type" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                    <option value="text">Teks</option><option value="image">Gambar</option><option value="video">Video</option><option value="document">Dokumen</option>
                </select>
            </div>
            <div>
                ...
                <select name="session_id" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
                    ...
                    <?php $__currentLoopData = $sessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($s->id); ?>"><?php echo e($s->name); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Header Text</label>
            <input type="text" name="header_text" value="<?php echo e(old('header_text')); ?>" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Header Media URL (opsional)</label>
            <input type="url" name="header_media_url" value="<?php echo e(old('header_media_url')); ?>" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Body Text</label>
            <textarea name="body_text" rows="3" required class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm"><?php echo e(old('body_text')); ?></textarea>
        </div>
        <div>
            <label class="text-xs font-medium text-gray-500">Footer Text (opsional)</label>
            <input type="text" name="footer_text" value="<?php echo e(old('footer_text')); ?>" class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm">
        </div>
        <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\buttons\create.blade.php ENDPATH**/ ?>