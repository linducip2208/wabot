<?php $__env->startSection('title', __('instagram.title') . ' — WABot'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-5xl mx-auto">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900"><?php echo e(__('instagram.title')); ?></h1>
            <p class="text-sm text-gray-500 mt-0.5"><?php echo e(__('instagram.subtitle')); ?></p>
        </div>
        <button onclick="document.getElementById('addModal').classList.remove('hidden')"
            ...
            <i class="fas fa-plus text-xs"></i> <?php echo e(__('instagram.create_account')); ?>
    ...
    <?php if($accounts->isEmpty()): ?>
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <h3 class="text-gray-500 font-medium mb-1"><?php echo e(__('instagram.empty_title')); ?></h3>
            <p class="text-sm text-gray-400 mb-4"><?php echo e(__('instagram.empty_desc')); ?></p>
            <button onclick="document.getElementById('addModal').classList.remove('hidden')"
                class="bg-pink-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-pink-700 transition">
                <?php echo e(__('instagram.create_account')); ?>
                    ...
                    <div class="flex items-start justify-between flex-wrap gap-4">
                        ...
                        <div class="flex items-center gap-1.5">
                            <?php if($acc->status !== 'connected'): ?>
                                <form action="<?php echo e(route('instagram.connect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    <button class="px-3.5 py-2 rounded-xl text-xs font-medium bg-pink-600 text-white hover:bg-pink-700 transition">
                                        <i class="fab fa-instagram mr-1"></i><?php echo e(__('common.connect')); ?>

                                    </button>
                                </form>
                            <?php else: ?>
                                <form action="<?php echo e(route('instagram.disconnect', $acc)); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                            ...
                            <?php endif; ?>
                            <form action="<?php echo e(route('instagram.destroy', $acc)); ?>" method="POST" onsubmit="return confirm('<?php echo e(__('common.delete')); ?>?')" class="inline">
    ...
    <div class="mt-4 bg-pink-50 rounded-xl border border-pink-100 p-5">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h3 class="font-semibold text-gray-900 text-sm mb-1"><?php echo e(__('instagram.webhook_url')); ?></h3>
                <p class="text-xs text-gray-500 mb-2"><?php echo e(__('instagram.webhook_hint')); ?></p>
                <code class="inline-block bg-white border border-pink-200 rounded-lg px-3 py-1.5 text-xs font-mono text-pink-700 break-all">
                    <?php echo e(route('webhook.instagram')); ?>
...
<div id="addModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold text-gray-900 mb-4"><?php echo e(__('instagram.create_account')); ?></h2>
        <form action="<?php echo e(route('instagram.store')); ?>" method="POST" class="space-y-3">
            <?php echo csrf_field(); ?>
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('meta.app_id')); ?></label>
                <input type="text" name="app_id" placeholder="Dari Meta Developer" required
                    class="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo e(__('meta.app_secret')); ?></label>
            ...
            <div class="flex gap-2 pt-1">
                ...
                <button type="submit" class="flex-1 bg-pink-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-pink-700"><?php echo e(__('common.save')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\instagram\index.blade.php ENDPATH**/ ?>