<?php $__env->startSection('title', 'Payment Gateway — Admin'); ?>
...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5"><?php echo e($gateways->count()); ?> gateway tersedia</p>
    </div>
    <button onclick="openCreate()"
        ...
        <i class="fas fa-plus text-xs"></i> <?php echo e(__('common.create')); ?> Gateway
    </button>
</div>
...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 card-lift <?php echo e($g->is_active ? '' : 'opacity-50'); ?>">
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs" style="background: <?php echo e($g->logo_color ?? ($driverColors[$g->driver] ?? '#3b82f6')); ?>">
                    <i class="fas <?php echo e($driverIcons[$g->driver] ?? 'fa-money-check'); ?>"></i>
                </div>
                <div>
                    <span class="font-semibold text-sm text-gray-900"><?php echo e($g->name); ?></span>
                    <?php if($g->driver): ?>
                        <span class="block text-[10px] text-gray-400 uppercase"><?php echo e($drivers[$g->driver] ?? $g->driver); ?></span>
                    <?php endif; ?>
                </div>
            </div>
            <span class="text-[10px] px-1.5 py-0.5 rounded font-medium <?php echo e($g->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'); ?>">
                <?php echo e($g->is_active ? __('common.active') : __('common.inactive')); ?>

            </span>
        </div>
        <div class="text-xs text-gray-500 space-y-0.5 mb-3">
            <?php if($g->account_number): ?><div><span class="text-gray-400">Rek:</span> <?php echo e($g->account_number); ?></div><?php endif; ?>
            <?php if($g->account_holder): ?><div><span class="text-gray-400"><?php echo e(__('common.name')); ?>:</span> <?php echo e($g->account_holder); ?></div><?php endif; ?>
            <?php if($g->is_auto): ?><div class="text-emerald-600"><i class="fas fa-bolt mr-1"></i>Auto-capture</div><?php endif; ?>
        </div>
        <div class="text-[11px] text-gray-400 bg-gray-50 rounded-lg p-2 max-h-20 overflow-y-auto whitespace-pre-line mb-3"><?php echo e(\Str::limit($g->instructions, 80)); ?></div>
        <div class="flex gap-1">
            <button onclick="editGateway(<?php echo e($g->id); ?>)"
                class="flex-1 text-[11px] bg-gray-100 text-gray-700 hover:bg-gray-200 rounded-lg py-1.5 font-medium text-center"><?php echo e(__('common.edit')); ?></button>
            <form method="POST" action="<?php echo e(route('admin.gateways.destroy', $g)); ?>" class="flex-1" onsubmit="return confirm('<?php echo e(__('common.delete')); ?>?')">
                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                <button class="w-full text-[11px] bg-red-50 text-red-600 hover:bg-red-100 rounded-lg py-1.5 font-medium"><?php echo e(__('common.delete')); ?></button>
            </form>
        </div>
    </div>
...
<div id="gwModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
        <h2 class="text-lg font-bold mb-4" id="gwModalTitle"><?php echo e(__('common.create')); ?> Gateway</h2>
        <form method="POST" action="<?php echo e(route('admin.gateways.store')); ?>" class="space-y-3" id="gwForm">
            <?php echo csrf_field(); ?>
            ...
            <div id="manualFields">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="text-xs font-medium text-gray-500"><?php echo e(__('common.name')); ?> Pemilik</label>
                ...
                <div class="mt-3">
                    <label class="text-xs font-medium text-gray-500">Instruksi <?php echo e(__('common.payment')); ?></label>
            ...
            <div id="apiFields" class="hidden">
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="gwIsAuto" class="text-sm text-gray-600">Auto-capture <?php echo e(__('common.payment')); ?></label>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-2 pt-1">
                <button type="button" onclick="document.getElementById('gwModal').classList.add('hidden')" class="flex-1 bg-gray-100 text-gray-700 rounded-xl py-2.5 text-sm font-medium"><?php echo e(__('common.cancel')); ?></button>
...
<script>
const gatewaysData = <?php echo json_encode($gateways->keyBy('id'), 15, 512) ?>;
...
function openCreate() {
    document.getElementById('gwModalTitle').textContent = '<?php echo e(__('common.create')); ?> Gateway';
    const f = document.getElementById('gwForm');
    f.action = '<?php echo e(route('admin.gateways.store')); ?>';
    f.reset();
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\project laravel\wabot\resources\views\admin\gateways\index.blade.php ENDPATH**/ ?>